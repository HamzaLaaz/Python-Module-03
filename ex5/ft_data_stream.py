...
def stream_analytics(list_stream):
    ...
    print(f"Item found events: {list_stream[2]}")
    print(f"Kill events: {list_stream[3]}")
    print(f"Level-up events: {list_stream[4]}\n")
...
def processing_game_events():
    ...
    for event in event_generator():
        ...
        if event['event_type'] == 'item_found':
            ...
            level_up += 1
    list_stream = [total_events_proc, hight_level_ply, event_item_found,
                   event_kill, level_up]
    stream_analytics(list_stream)
